...
def button_function(bot):
    ...
    async def update_basket_message(id_chat: int, id_message: int, index: int, basket):
        await bot.delete_message(id_chat, id_message)
        markup = basket_message(id_chat, index, basket)
    ...
    async def callback(call):
        ...
        if call.data.startswith("left_"):
            ...
            update_product_message(id_chat, call.message.message_id, index, products)
            ...
            if status == 200:
                ...
                users[call.message.chat.id]["basket"] = await get_bascket(call.message.chat.id, bot)
            else:
        ...
        # Basket
        elif call.data.startswith("left_basket_"):
            ...
            users[id_chat]["index_basket"] = index
            await update_basket_message(id_chat, call.message.message_id, index, basket)
        elif call.data.startswith("plus_basket_") and users[id_chat]["basket"][index].count <= users[id_chat]["basket"][index].product_count:
            index = users[id_chat]["index_basket"]
            basket = users[id_chat]["basket"]
            users[id_chat]["basket"][index].count += 1
            await update_bascket(id_user=call.message.message_id, id_basket=basket[index].id, count = users[id_chat]["basket"][index].count)
            await update_basket_message(id_chat, call.message.message_id, index, basket)
        elif call.data.startswith("minus_basket_") and users[id_chat]["basket"][index].count > 0:
            index = users[id_chat]["index_basket"]
            basket = users[id_chat]["basket"]
            users[id_chat]["basket"][index].count -= 1
            await update_basket_message(id_user=call.message.message_id, id_basket=basket[index].id, count = users[id_chat]["basket"][index].count)
            await update_product_message(id_chat, call.message.message_id, index, basket)
        elif call.data.startswith("delete_basket_"):
            index = users[id_chat]["index_basket"]
            basket = users[id_chat]["basket"]
            status = await delete_bascket(id_user=call.message.message_id, id_basket=basket[index].id)
            if status == 200:
                bot.send_message(call.id, "Добавлено в корзину!")
            else:
                bot.send_message(call.id, "Ошибка при добавлении в корзину.")
            ...
            index = users[id_chat]["favorites_index"]
            favorites = users[id_chat]["favorites"]
            status = await delete_favorites(id_user=call.message.message_id, id_favorites=favorites[index].id)
            if status == 200:
                bot.send_message(call.id, "Удалено из избранного!")
            else:
